...
数据准备只接受可读的 JPG、JPEG、PNG、BMP、WEBP 图片，默认按照 70% / 15% / 15% 切分，并使用随机种子 `42` 保持可复现。训练数据的唯一权威来源为 [Mendeley Data: bdd69gyhv8.1](https://data.mendeley.com/datasets/bdd69gyhv8/1)，许可为 CC BY 4.0，引用 DOI 为 `10.17632/bdd69gyhv8.1`；使用或再分发数据时应保留原始署名、许可和 DOI。
