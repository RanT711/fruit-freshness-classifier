...
数据准备只接受可读的 JPG、JPEG、PNG、BMP、WEBP 图片，默认按照 70% / 15% / 15% 切分，并使用随机种子 `42` 保持可复现。公开数据来源与许可信息应随数据保留，可参考 [Project-AgML 数据集页](https://huggingface.co/datasets/Project-AgML/fresh_rotten_fruit_classification) 和 [Mendeley 原始记录](https://data.mendeley.com/datasets/bdd69gyhv8/1)。
...
- 网页上传仅接受 `jpg/jpeg/png`，原始字节大小上限为 10 MiB，像素总数上限为 16,000,000；
