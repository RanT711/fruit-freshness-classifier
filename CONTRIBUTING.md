...
- 未确认来源许可的数据、处理结果或数据集压缩包；
- 临时下载文件、探测文件和无关训练产物，例如 `models/best.pt.part`、`data/raw/*.bin`、`data/raw/*.zip`；
- 例外：仓库已明确跟踪的 `models/best.pt` 是本项目公开演示权重，其他模型权重仍不应提交；
- 例外：仓库现有的 `data/` 和 `runs/` 内容是已核验的公开数据与训练记录；新增内容仍需确认许可、清除个人信息并评估体积；
